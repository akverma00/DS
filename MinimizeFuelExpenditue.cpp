...
Also it would be really helpful, if someone would share the resources/links of variations of this question.
...
ll dijastra(ll n, ll s, ll d, vector<vector<pair<ll, ll>>> &egg, ll cap, vll &prices, set<ll> &se)
{
    ...
    while (!pq.empty())
    {
        ...
        for (auto &vvv : egg[u])
        {
            ...
            if (se.find(u) != se.end() && cap_now + 1 <= cap)
            {
...
void solve()
{
    ll n, m, src, s, q, k, a, b, dest, c, cap;
    cin >> n >> m >> s;
    cin >> cap;
    ...
    a--;
    b--;
    ll ans = dijastra(n, a, b, egg, cap, prices, se);
    cout << ans;
    cout << "\n";
}
int main()
